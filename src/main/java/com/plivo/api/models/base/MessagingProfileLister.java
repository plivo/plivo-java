...
public abstract class MessagingProfileLister<T extends BaseResource> extends BaseRequest<T> implements Iterable<T> {
  ...
  @Override
  public MessagingProfileLister<T> client(final PlivoClient plivoClient) {
   ...
   */                                                                                                    
  public MessagingProfileLister<T> limit(final Integer limit) {                                                     
    this.limit = limit;                                                                                  
    return this;                                                                                         
  }                                                                                                      
   ...
   */                                                                                                    
  public MessagingProfileLister<T> offset(final Integer offset) {                                                   
    this.offset = offset;                                                                                
    return this;                                                                                         
  }                                                                                                      
  ...
  public Iterator<T> iterator() throws IterableError {                                                   
    ...
    return new Iterator<T>() {                                                                           
      ...
      public boolean hasNext() {                                                                         
        ...
        try {                                                                                            
            ListResponse<T> itemList = MessagingProfileLister.this.list();                                       
          if (itemList.getProfiles().isEmpty()) {                                                     
            return false;                                                                              
          }                                                                                            
          this.items.addAll(itemList.getProfiles());                                                  
          offset += limit;                                                                             
